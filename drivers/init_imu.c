...
s8 _imu_i2c_read(u8 dev_address, u8 reg_address, u8 *arr_data, u8 count)
{
  ...
  else
  {
    ...
    for(i = 0; i < count; i++)
    {
      ...
      {
        comres = -1; // error occurred, end comms and exit
        I2CMasterControl(I2C0_BASE, I2C_MASTER_CMD_BURST_RECEIVE_FINISH);
        while(I2CMasterBusy(I2C0_BASE));
        i = count;
      }
...
struct bno055_accel_float_t get_linear_acceleration(){
    struct bno055_accel_float_t xyz_float;
    error += bno055_convert_float_accel_xyz_msq(&xyz_float);
    return xyz_float;
}
